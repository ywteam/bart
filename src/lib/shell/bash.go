...
// universal installer
func BashUInstaller() string {
	return `
		uinstaller:pkgmgr() {
			for pkgmgr in {apt-get,yum,dnf,brew,pacman}; do
		...
		uinstaller:remove(){
			...
			command -v $1 &>/dev/null || return 3
			args+=($1)
			$cmd ${args[@]} &>/dev/null
			[[ $? -ne 0 ]] && unset cmd args && return 4
		...
		uinstaller:install(){
			...
			$cmd ${args[@]} &>/dev/null
			[[ $? -ne 0 ]] && unset cmd args && return 3
			for arg in $@; do
				! command -v $arg &>/dev/null && unset cmd args && return 4
			done
			return 0
		}
	`
...
var Cache = func() string {
	chunks, err := os.ReadFile("/workspace/projects/ydk/src/opsh/dist/sdk.sh")
	if err != nil {
		return ""
	}
	return string(chunks)
}
var Download = func() string {
	return ``
	// resp, err := http.Get("https://github.com/ywteam/opsh/releases/latest")
	// if err != nil {
	// 	return ""
	// }
